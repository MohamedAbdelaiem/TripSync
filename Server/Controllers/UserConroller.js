...
exports.getUser = async (req, res) => {
  try {
    ...
    client.query(
      "SELECT email,ProfilePhoto,profilename,role,username FROM users WHERE user_id=$1",
      [user_id],
      (err, result) => {
        if (err) {
          console.log(err);
          ...
          if (role === "travelAgency") {
            client.query(
              "SELECT t.Location, t.Address, t.PhoneNumber, t.Email, t.Rate,t.Description,t.Country,s.email,s.profilephoto,s.profilename FROM travelagency AS t,users AS s WHERE TravelAgency_ID=user_id AND user_id=$1",
              [user_id],
              (err, result) => {
                if (err) {
                  console.log(err);
            ...
            client.query(
              "SELECT email,profilephoto,profilename,role,username,t.Points,t.NumberOfTrips FROM traveller AS t,users WHERE traveller_id=user_id AND user_id=$1",
              [user_id],
              (err, result) => {
                if (err) {
                  console.log(err);
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    if (role === "traveller") {
      ...
      const { phoneNumber, location, address, email, description, country } =
        req.body;
...
exports.DeleteUser = async (req, res) => {
  try {
    ...
    const rows = await client.query("DELETE FROM users WHERE user_id=$1", [
      user_id,
    ]);
    await client.query("COMMIT");
    if (rows.rowCount === 0) {
...
exports.UpdateMe = async (req, res) => {
  try {
    ...
    if (profilephoto) {
      try {
        await client.query(
          "UPDATE users SET profilephoto=$1 WHERE user_id=$2",
          [profilephoto, req.params.user_id]
        );
      } catch (err) {
        res.json({
          success: false,
    ...
    if (profilename) {
      try {
        await client.query("UPDATE users SET profilename=$1 WHERE user_id=$2", [
          profilename,
          req.params.user_id,
        ]);
      } catch (err) {
    ...
    if (newPassword) {
      if (!previousPassword) {
        ...
        try {
          await client.query("SELECT * FROM users WHERE user_id=$1", [
            req.params.user_id,
          ]);
